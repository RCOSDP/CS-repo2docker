...
def test_provisioner_resolve_source_missing_storage():
    project = MagicMock()
    project.storages = AsyncIterator([])
    provisioner = Provisioner(project, "osfstorage")
    path_mapping = MagicMock(spec=PathMapping)
    path_mapping.get_source.return_value = "unknownstorage/subdir"

    async def runner():
        await provisioner._resolve_source(path_mapping)

    with pytest.raises(ValueError, match="Could not find storage unknownstorage"):
        asyncio.run(runner())


def test_provisioner_resolve_source_missing_path():
    storage = MagicMock()
    storage.name = "osfstorage"
    project = MagicMock()
    project.storages = AsyncIterator([storage])
    provisioner = Provisioner(project, "osfstorage")
    path_mapping = MagicMock(spec=PathMapping)
    path_mapping.get_source.return_value = "osfstorage/missing"

    async def runner():
        with patch(
            "repo2docker.contentproviders.rdm.provisioner.find_by_path",
            new=AsyncMock(return_value=None),
        ):
            await provisioner._resolve_source(path_mapping)

    with pytest.raises(ValueError, match="Could not find path osfstorage/missing"):
        asyncio.run(runner())
